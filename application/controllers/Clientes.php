...
class Clientes extends CI_Controller
{
    ...
    public function edit($cliente_id = NULL)
    {
        ...
        if (!$cliente_id || !$this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))) {
            ...
            if ($cliente_tipo == 1) {
                $this->form_validation->set_rules('cliente_cpf', '', 'trim|required|min_length[10]|max_length[20]|callback_check_cpf');
            } else {
                $this->form_validation->set_rules('cliente_cnpj', '', 'trim|required|min_length[10]|max_length[20]|callback_check_cnpj');
            }
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->db->table_exists('contas_receber')) {
                    if ($cliente_ativo == 0 && $this->core_model->get_by_id('contas_receber', array('conta_receber_cliente_id' => $cliente_id, 'conta_receber_status' => 0))) {
                        $this->session->set_flashdata('error', 'Este cliente não pode ser desativado pois está com contas em aberto!');
                        redirect('clientes');
                    }
                }
    ...
    public function novo()
    {
        ...
        if ($cliente_tipo == 1) {
            $this->form_validation->set_rules('cliente_cpf', '', 'trim|required|min_length[10]|max_length[20]|is_unique[clientes.cliente_cpf_cnpj]|callback_check_cpf');
        } else {
            $this->form_validation->set_rules('cliente_cnpj', '', 'trim|required|min_length[10]|max_length[20]|is_unique[clientes.cliente_cpf_cnpj]|callback_check_cnpj');
        }
